...
def get_iterator(
    ...
    _src_ids, _tgt_input_ids, _tgt_output_ids, _src_seq_len, _tgt_seq_len = batched_iter.get_next()
    src_ids = tf.split(_src_ids, num_or_size_splits=num_splits, axis=0)
    tgt_input_ids = tf.split(_tgt_input_ids, num_or_size_splits=num_splits, axis=0)
    tgt_output_ids = tf.split(_tgt_output_ids, num_or_size_splits=num_splits, axis=0)
    src_seq_len = tf.split(_src_seq_len, num_or_size_splits=num_splits, axis=0)
    tgt_seq_len = tf.split(_tgt_seq_len, num_or_size_splits=num_splits, axis=0)
    """
