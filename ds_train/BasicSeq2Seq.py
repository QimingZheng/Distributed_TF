...
class Seq2SeqModel(object):
    ...
    def _single_cell(self, unit_type, num_units, forget_bias, dropout, mode, residual_connection=False):
        ...
        if unit_type == "lstm":
            single_cell = tf.contrib.rnn.LSTMCell(
                num_units, initializer=tf.orthogonal_initializer(), forget_bias=forget_bias)
        elif unit_type == "bn_lstm":
            single_cell = BN_LSTM.BNLSTMCell(
                num_units, forget_bias=forget_bias, normalize_in_to_hidden=True)
        elif unit_type == "wn_lstm":
            single_cell = contrib_rnn_cell.WeightNormLSTMCell(num_units)
        elif unit_type == "ln_lstm":
        ...
        for i in range(num_layers):
            cell_list.append(
                self._single_cell(
                    ...
                    dropout=dropout,
                    mode=mode,
                    residual_connection= (residual_connection and i>0),
                ))
    ...
    def _build_bidirectional_rnn(
        ...
        fw_cell = self._build_rnn_cell(num_layers, unit_type, num_units,
                                       forget_bias, dropout, mode, residual_connection)
        ...
        with tf.variable_scope("encoder") as scope:
            ...
            if hparams.encoder_type == "cudnn_lstm":
                ...
                cell = self._build_rnn_cell(
                    ...
                    dropout=hparams.dropout,
                    mode=self.mode,
                    residual_connection=hparams.use_residual_connection,
                )
            ...
            if hparams.encoder_type == "cudnn_lstm":
                ...
                (outputs, final_context_state,
                 _) = tf.contrib.seq2seq.dynamic_decode(
                     ...
                     2 * tf.reduce_max(source_sequence_length),
                 )
                if self.mode != tf.contrib.learn.ModeKeys.INFER:
